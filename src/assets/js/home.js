...
class Home extends BasePage {
    ...
    async loadAutoFP5() {
        ...
        const title   = section.dataset.title || 'الأكثر مبيعاً';
        let products  = null;

        // 1 — try Salla JS SDK if available
        try {
            if (salla?.api?.get) {
                const r = await salla.api.get('/store/products', { sort_by: 'most_sales', per_page: 4 });
                products = r?.data?.products || r?.products;
            }
        } catch { /* fall through */ }

        // 2 — fall back to direct store REST API calls
        if (!products?.length && apiBase) {
            const paths = [
                '/v1/products?sort_by=most_sales&per_page=4',
                '/products?sort_by=most_sales&per_page=4',
                '/v1/products?per_page=4',
                '/products?per_page=4',
            ];
            for (const path of paths) {
                try {
                    ...
                    if (Array.isArray(p) && p.length) { products = p; break; }
                } catch { /* next */ }
            }
        }
        ...
        section.innerHTML = this.buildFP5Html(title, products.slice(0, 4));
    }

    buildFP5Html(title, products) {
        ...
        const cards = products.map(p => {
            ...
            const badge = isOnSale && p.discount_percentage
                ? `<span class="fp5-badge">${p.discount_percentage}</span>` : '';
            const stars = p.rating?.count > 0
